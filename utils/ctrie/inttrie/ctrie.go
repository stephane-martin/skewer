...
func lNodeLookup(l *list, key string) (*int32, bool) {
...
func lNodeInserted(l *list, key string, value *int32, hash uint32) *list {
...
func (c *Ctrie) Insert(key string, value *int32) {
	c.assertReadWrite()
...
func (c *Ctrie) insert(key string, value *int32, hash uint32) {
	root := c.readRoot()
	for {
...
func (c *Ctrie) iinsert(i *iNode, key string, value *int32, hash uint32, lev uint, parent *iNode, startGen *generation) bool {
	// Linearization point.
	main := gcasRead(i, c)
	switch {
	case main.cNode != nil:
