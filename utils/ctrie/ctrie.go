...
func (l *lNode) lookup(e *Entry) (Data, bool) {
	found, ok := l.Find(func(sn *sNode) bool {
		return bytes.Equal(e.Key, sn.Key)
	})
	if !ok {
		return nil, false
	}
...
func (c *Ctrie) Insert(key []byte, value Data) {
	c.assertReadWrite()
	c.insert(&Entry{
		Key:   key,
		Value: value,
...
func (c *Ctrie) Size() (size uint) {
	...
	snapshot := c.ReadOnlySnapshot()
	snapshot.count(snapshot.readRoot(), &size)
	return size
}

var errCanceled = errors.New("canceled")

func (c *Ctrie) count(i *iNode, ch *uint) {
	main := gcasRead(i, c)
	switch {
	case main.cNode != nil:
...
func (c *Ctrie) lookup(entry *Entry) (Data, bool) {
	root := c.readRoot()
...
func (c *Ctrie) remove(entry *Entry) (Data, bool) {
	root := c.readRoot()
...
// should be retried.
func (c *Ctrie) ilookup(i *iNode, entry *Entry, lev uint, parent *iNode, startGen *generation) (Data, bool, bool) {
	// Linearization point.
	main := gcasRead(i, c)
	switch {
	case main.cNode != nil:
...
// Ctrie. The last bool indicates if the operation succeeded. False means it
// should be retried.
func (c *Ctrie) iremove(i *iNode, entry *Entry, lev uint, parent *iNode, startGen *generation) (Data, bool, bool) {
	// Linearization point.
	main := gcasRead(i, c)
	switch {
	case main.cNode != nil:
...
func cleanReadOnly(tn *tNode, lev uint, p *iNode, ctrie *Ctrie, entry *Entry) (val Data, exists bool, ok bool) {
	if !ctrie.readOnly {
		clean(p, lev-5, ctrie)
		return nil, false, false
	}
...
type PersistentList interface {
	...
	// item which matches.
	Find(func(*sNode) bool) (*sNode, bool)
...
// Head returns the head of the list. The bool will be false if the list is
// empty.
func (e *emptyList) Head() (*sNode, bool) {
	return nil, false
}
...
// invalid.
func (e *emptyList) Get(pos uint) (*sNode, bool) {
	return nil, false
}
...
// which matches.
func (e *emptyList) Find(func(*sNode) bool) (*sNode, bool) {
	return nil, false
}
...
func (e *emptyList) FindIndex(func(*sNode) bool) int {
	return -1
}

// Map applies the function to each entry in the list and returns the resulting
// slice.
func (e *emptyList) Map(func(*sNode) *Entry) []*Entry {
	return nil
}
...
// Add will add the item to the list, returning the new list.
func (l *list) Add(head *sNode) PersistentList {
	return &list{head, l}
}

// Insert will insert the item at the given position, returning the new list or
// an error if the position is invalid.
func (l *list) Insert(val *sNode, pos uint) (PersistentList, error) {
	if pos == 0 {
...
// Get returns the item at the given position or an error if the position is
// invalid.
func (l *list) Get(pos uint) (*sNode, bool) {
	if pos == 0 {
		return l.head, true
	}
...
// FindIndex applies the predicate function to the list and returns the index
// of the first item which matches or -1 if there is no match.
func (l *list) FindIndex(pred func(*sNode) bool) int {
	curr := l
	idx := 0
	for {
		if pred(curr.head) {
...
// Map applies the function to each entry in the list and returns the resulting
// slice.
func (l *list) Map(f func(*sNode) *Entry) []*Entry {
	cur := l
	entries := make([]*Entry, 0, l.Length())
	for {
		entries = append(entries, f(cur.head))
		if cur.tail.IsEmpty() {
			break
		}
		cur = cur.tail.(*list)
