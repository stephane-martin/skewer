...
type rawudpnode struct {
	position uint64
	data     *model.RawUdpMessage
}

type rawudpnodes []*rawudpnode

type RawUdpRing struct {
	_padding0      [8]uint64
	queue          uint64
	_padding1      [8]uint64
	dequeue        uint64
...
func (rb *RawUdpRing) init(size uint64) {
	size = roundUp(size)
	rb.nodes = make(rawudpnodes, size)
	for i := uint64(0); i < size; i++ {
		rb.nodes[i] = &rawudpnode{position: i}
	}
...
func (rb *RawUdpRing) Put(item *model.RawUdpMessage) error {
...
// queue is disposed.
func (rb *RawUdpRing) Offer(item *model.RawUdpMessage) (bool, error) {
...
func (rb *RawUdpRing) Get() (*model.RawUdpMessage, error) {
	return rb.Poll(0)
}
...
func (rb *RawUdpRing) Poll(timeout time.Duration) (*model.RawUdpMessage, error) {
	var (
		n     *rawudpnode
