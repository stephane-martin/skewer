...
// Ring is a thread-safe bounded queue that stores *model.RawKafkaMessage messages.
type Ring struct {
	mask      uint64
	_padding0 [8]uint64
	queue     atomic.Uint64
...
func (rb *Ring) Put(item *model.RawKafkaMessage) error {
...
// queue is disposed.
func (rb *Ring) Offer(item *model.RawKafkaMessage) (bool, error) {
...
func (rb *Ring) Get() (*model.RawKafkaMessage, error) {
	return rb.Poll(0)
}

func (rb *Ring) PollDeadline(deadline time.Time) (*model.RawKafkaMessage, error) {
...
func (rb *Ring) Poll(timeout time.Duration) (*model.RawKafkaMessage, error) {
	var (
		n     *node
		pos   = rb.dequeue.Load()
		start time.Time
		w     utils.ExpWait
		zero  *model.RawKafkaMessage
	)
	if timeout > 0 {
		start = time.Now()
	}
