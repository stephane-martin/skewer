...
// Ring is a thread-safe bounded queue that stores *model.RawTcpMessage messages.
type Ring struct {
	mask      uint64
	_padding0 [8]uint64
	queue     atomic.Uint64
...
func (rb *Ring) Put(item *model.RawTcpMessage) error {
...
// queue is disposed.
func (rb *Ring) Offer(item *model.RawTcpMessage) (bool, error) {
	return rb.put(item, true)
}

func (rb *Ring) put(item *model.RawTcpMessage, offer bool) (bool, error) {
	var (
		n *node
		w utils.ExpWait
	)
...
func (rb *Ring) Get() (*model.RawTcpMessage, error) {
	return rb.Poll(0)
}

func (rb *Ring) PollDeadline(deadline time.Time) (*model.RawTcpMessage, error) {
...
func (rb *Ring) Poll(timeout time.Duration) (*model.RawTcpMessage, error) {
	var (
		n     *node
		pos   = rb.dequeue.Load()
		start time.Time
		w     utils.ExpWait
		zero  *model.RawTcpMessage
	)
	if timeout > 0 {
		start = time.Now()
	}
