...
// Ring is a thread-safe bounded queue that stores utils.UIDString messages.
type Ring struct {
	mask      uint64
	_padding0 [8]uint64
	queue     atomic.Uint64
...
func (rb *Ring) Put(item utils.UIDString) error {
...
// queue is disposed.
func (rb *Ring) Offer(item utils.UIDString) (bool, error) {
	return rb.put(item, true)
}

func (rb *Ring) put(item utils.UIDString, offer bool) (bool, error) {
	var n *node
	w := waiter.Default()
	pos := rb.queue.Load()
...
func (rb *Ring) Get() (utils.UIDString, error) {
	return rb.Poll(0)
}

func (rb *Ring) PollDeadline(deadline time.Time) (utils.UIDString, error) {
...
func (rb *Ring) Poll(timeout time.Duration) (utils.UIDString, error) {
	var (
		n     *node
		pos   = rb.dequeue.Load()
		start time.Time
		zero  utils.UIDString
	)
	w := waiter.Default()
	if timeout > 0 {
		start = time.Now()
