...
type Entry struct {
	...
	TerminationStatus int       `json:"termination_status"`
...
func (e *Entry) String() string {
	return fmt.Sprintf(
		"Type: '%s', PID: '%d', Line: '%s', ID: '%s', User: '%s', Host: '%s', Termination: '%d', Exit: '%d', Session: '%d', Timestamp: '%s'",
		e.TypeStr, e.PID, e.Line, e.ID, e.User, e.Host, e.TerminationStatus, e.ExitStatus, e.SessionID, e.Timestamp,
	)
}
...
func next() (entry *Entry) {
	...
	entry = &Entry{
		...
		TerminationStatus: centry.ut_exit.e_termination,
		ExitStatus:        centry.ut_exit.e_exit,
		SessionID:         centry.ut_session,
